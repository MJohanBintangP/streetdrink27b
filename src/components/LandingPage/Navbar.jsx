...
const Navbar = () => {
  ...
  return (
    <nav className="relative z-10 mx-auto max-w-[1350px] px-4 py-6 flex items-center justify-between select-none">
      <img
        src={logo}
        className="w-[150px] sm:w-[180px] pointer-events-none"
        alt="Logo"
        loading="eager"
        fetchPriority="high"
        decoding="async"
      ...
      <div className="flex items-center gap-4 sm:hidden">
        ...
        <button
          className="cursor-pointer text-[#3D91FF] text-2xl"
          onClick={toggleMenu}
        >
          {menuOpen ? (
        ...
        {NAV_ITEMS.map((item) => (
          <li key={item.id} className="py-2 sm:py-0 text-center sm:text-left">
            <button
              ...
              className="cursor-pointer w-full block sm:inline-block select-none"
            >
              {item.label}
            </button>
          </li>
      ...
      <div className="hidden sm:flex gap-6 text-[12px] text-[#181E6A] select-none">
        ...
        <button
          ...
          className="hover:scale-105 transition-transform cursor-pointer rounded-[10px] bg-[#3D91FF] px-6 py-2 text-white drop-shadow-[4px_5px_1px_rgba(0,0,0,0.25)]"
        >
          Kontak
        </button>
      </div>
